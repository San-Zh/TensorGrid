...
void TensorGrid_CXTY(TF *dest, TF *X, TF *Y, size_t tensorSize, const int gridSize);
...
void ComplexAry_MatrixVector_v2(Tp *dest, const Tp *mat, const Tp *src, const int gridSize)
{
    for (size_t v = 0; v < gridSize; v++) {
        for (size_t row = 0; row < M; row++) {
            ...
            // for (size_t col = 0; col < N; col++) {
            //     res_re +=
            //         mat[v * 2 * M * N + row * 2 * N + 2 * col + 0] * src[v * 2 * N + 2 * col + 0] -
            //         mat[v * 2 * M * N + row * 2 * N + 2 * col + 1] * src[v * 2 * N + 2 * col + 1];
            //     res_im +=
            //         mat[v * 2 * M * N + row * 2 * N + 2 * col + 0] * src[v * 2 * N + 2 * col + 1] +
            //         mat[v * 2 * M * N + row * 2 * N + 2 * col + 1] * src[v * 2 * N + 2 * col + 0];
            // }
            Tp vsrc[N][2];
            for (size_t col = 0; col < N; col++) {
